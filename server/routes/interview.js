...
router.post('/start', async (req, res) => {
  try {
    ...
    Job: ${job.title}, Skills: ${job.skillsRequired.join(', ')}.
    Return ONLY a JSON array of strings, no extra text:
    ["question1", "question2", "question3", "question4", "question5"]
    `;
    ...
    try {
      ...
      let cleanResponse = textOutput.replace(/^```json\s*|\s*```$/g, '');
      questions = JSON.parse(cleanResponse);
    } catch (e) {
...
router.post('/submit/:interviewId', async (req, res) => {
  try {
    ...
    Be strict: each question holds equal weight.
    Return a JSON array like [score, "Short report about candidate fit"].
    `;
    ...
    const scoredQuestions = interview.questions.map((q, i) => ({
      question: q.question,
      answer: answers[i] || '',
